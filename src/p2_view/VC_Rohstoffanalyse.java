...
public class VC_Rohstoffanalyse {
	...
	private void handleIndifferenz() throws IOException {
		...
		for(double j = 0; j < 1; j+=0.01) {
			if(functp == "Linear") {
				series2.add(j, (-0.10+Schwellenwert) + averco/20 * j);
				series3.add(j, (-0.05+Schwellenwert) + averco/20 * j);
				series4.add(j, (Schwellenwert) + averco/20 * j);
				series5.add(j, (0.05+Schwellenwert) + averco/20 * j);
